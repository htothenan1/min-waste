...
const RecipesList = ({ recipes, handleSelectRecipe }) => {
  return (
    <div className="m-6 flex flex-col text-left">
      ...
      <ul
        role="list"
        className="flex flex-col h-96 bg-[conic-gradient(at_bottom_left,_var(--tw-gradient-stops))] from-slate-300/50 via-slate-100/50 to-indigo-100/50 shadow-2xl rounded-lg overflow-scroll w-96"
      >
        {recipes && recipes.length > 0 ? (
          ...
          <p className="p-2 cursor-default my-auto mx-auto text-slate-600 font-quicksand">
            No current recipes
          </p>
        )}
      </ul>
